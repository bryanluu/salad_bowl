...
function TurnScreen({ round, team, timeLeft, currentWord, bowlLength, onSkip, onWin }: TurnScreenProps) {
  ...
  function followCursor(event: React.PointerEvent) {
    ...
    if (!container || dragStartXRef.current === undefined) return
    ...
    const maxOffset = midpoint - horizontalGap // rail edge, in delta terms

    const delta = event.clientX - dragStartXRef.current
    const knobX = Math.min(Math.max(delta, -maxOffset), maxOffset)
    const progress = Math.min(Math.max(knobX / threshold, -1), 1)
    const onLastWord = bowlLength === 0

    setKnobOffsetX(midpoint + knobX) // still an absolute rail position for rendering
